...
这是一个用于 Codex 的技能，用来抓取微信公众号文章、保存为本地 HTML、启动本地文章服务，并为后续通过[花生壳内网穿透](http://url.oray.com/i/47632)映射公网、交给豆包 APP 转成播客做准备。

## 能做什么

- 抓取 `https://mp.weixin.qq.com/s/...` 形式的微信公众号文章，并保存为 HTML。
- 从本地 `articles/` 目录提供已保存文章的访问服务。
- 提供一套可复用流程，将本地文章服务通过[花生壳内网穿透](http://url.oray.com/i/47632)映射为公网 URL。
...
2. 启动本地文章服务。
3. 使用[花生壳内网穿透](http://url.oray.com/i/47632)，将本地服务映射为公网 URL。
